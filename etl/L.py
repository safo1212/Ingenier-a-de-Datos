...
def load(df: pd.DataFrame, connection: Engine, table_name: str, replace: bool = False) -> None:
    ...
    """
    if df.empty:
        return  # No intentar cargar DataFrames vacíos

    # Eliminar columnas completamente vacías por seguridad
    ...
    if replace:
        ...
        # Si la tabla no existe, se creará automáticamente por `to_sql`
        if_exists = 'append'
    else:
        if_exists = 'append'
